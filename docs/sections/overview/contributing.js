...
const Section = ({ seed }) =>
  <section>
    ...
    <p>Fiona aims to be performant, robust, well documented, <Link href='/static/dependency-graph.svg'><a>well organised</a></Link>, <Link href='/static/reports/coverage/index.html'><a>well tested</a></Link> and <Link href="/static/reports/size/main.html"><a>small</a></Link> (full library is ~8KB core is ~3KB gzipped).</p>
    ...
    <p>Whilst there are very few dependencies (only <NPM name='randexp' />) in the fiona library, there are several used to aid development:</p>
    ...
    <p>Updating or creating a pull request will trigger a deployment using zeit's now service, so you can preview the suggested changes in a production like environment.</p>
  </section>
